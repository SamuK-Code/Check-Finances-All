...
const SyncScreen = () => {
  ...
  const handleManualSync = useCallback(async () => {
    ...
    setIsSyncing(true);
    try {
      // Aqui você chamaria o sync do seu ExpenseContext
  ...
  return (
    <ScrollView style={styles.container}>
      ...
      <View style={styles.statusCard}>
        ...
        {lastSyncTime && (
          <View style={styles.statusRow}>
            ...
            <Text style={styles.statusValue}>{new Date(lastSyncTime).toLocaleString('pt-BR')}</Text>
          </View>
        )}
      </View>
      ...
      <View style={styles.section}>
        ...
        <View style={styles.settingRow}>
          <View style={styles.settingInfo}>
            ...
            <Text style={styles.settingDesc}>Sincroniza automaticamente</Text>
          </View>
          <Switch
            value={autoSync}
            onValueChange={setAutoSync}
            trackColor={{ false: '#30363d', true: '#58a6ff' }}
            thumbColor={autoSync ? '#fff' : '#666'}
          />
        </View>
      </View>
      ...
      <View style={styles.section}>
        ...
        <TouchableOpacity style={styles.actionButton} onPress={handleManualSync} disabled={isSyncing}>
          <Text style={styles.actionIcon}>🔄</Text>
          <View style={styles.actionInfo}>
            <Text style={styles.actionTitle}>{isSyncing ? 'Sincronizando...' : 'Sincronizar Agora'}</Text>
...
const styles = StyleSheet.create({
  ...
  statusBadge: { paddingHorizontal: 10, paddingVertical: 4, borderRadius: 12, backgroundColor: '#30363d' },
  statusSynced: { backgroundColor: 'rgba(35, 197, 94, 0.2)' },
  statusSyncing: { backgroundColor: 'rgba(88, 166, 255, 0.2)' },
  statusError: { backgroundColor: 'rgba(248, 81, 73, 0.2)' },
